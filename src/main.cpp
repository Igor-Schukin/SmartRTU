...
int main() {
  ...
  try {
    ...
    if (config->Load(CONFIG_DESTINATION) == false) {
      std::cerr << StrNow() << "\t"
                << "\033[1;31m CONFIG DID NOT LOADED check your config.cfg "
                   "destination or if file exist!\033[0m\n";
    } else {
      std::cout << StrNow() << "\t"
    ...
    std::cout << StrNow() << "\t"
              << "***** INFOBORAD engine has finished\n";

  } catch (...) {
    std::cerr << StrNow() << "\t"
              << "\t~~~~~ INFOBORAD engine crashed!!!\n";
